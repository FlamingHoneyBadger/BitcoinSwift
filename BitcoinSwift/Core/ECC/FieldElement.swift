...
protocol Field  {
    var prime: BigInt { get set }
    var number: BigInt { get set }
...
struct FieldElement  : Field {
    ...
    public static func *(left: BigInt, right: FieldElement)  -> FieldElement {
        var num = (left * right.number)
        num = Helper.mod(num, right.prime)
        return FieldElement(num: num, prime:right.prime)
    }

    public static func ^^(num: FieldElement, power: BigInt)  -> FieldElement {
        let n =  power.modulus(num.prime-1)  // Helper.mod(power , (num.prime - 1))
        let number =  num.number.power(n, modulus: num.prime) // Helper.powMod(base: num.number, exponent: n, modulo: num.prime)
