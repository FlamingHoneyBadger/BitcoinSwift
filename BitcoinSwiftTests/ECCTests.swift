...
//        let key  = SecureBytes(bytes: GMPInteger(4242424242).magnitude.serialize().bytes)
...
//        let keyB = SecureBytes.init(bytes: GMPInteger(1).magnitude.serialize().bytes)
       // let p = PrivateKey(key: keyB)
        let message = "Satoshi Nakamoto"
//        let z = Helper.sha256(data:  message.data(using: .ascii)!)
       // let sig = p.sign(z: GMPInteger(BigUInt(z)))
       // print(sig.DERBytes().hexEncodedString())
